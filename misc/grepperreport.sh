#!/bin/sh
# Save grepper output as <div>ified html
# Steady on the regex if you want to modify this
VERSION=0.2
cat <<HEADER
<html>
    <head>
        <title>grepper report for grepper $*</title>
        <meta name="keywords" content="grepper static analysis report" />
        <!-- Shameless plug -->
        <meta name="description" content="Static analysis report generated using grepper - http://www.justanotherhacker.com" />
        <meta name="generator" content="grepperreport using grepper version $gver" />
        <style>
...
# We use grepper -Z to do exact matching as ext regex don't support non greedy matching
grepper -Z $* | \
